...
use {
    crate::errors::SubscriptionError,
    solana_program::{
...
pub fn assert_initialized<T: Pack + IsInitialized>(
    ...
    if !account.is_initialized() {
        Err(SubscriptionError::Uninitialized.into())
    } else {
        Ok(account)
    }
}
...
pub fn assert_owned_by(account: &AccountInfo, owner: &Pubkey) -> ProgramResult {
    if account.owner != owner {
        ...
        Err(SubscriptionError::IncorrectOwner.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_rent_exempt(rent: &Rent, account_info: &AccountInfo) -> ProgramResult {
    if !rent.is_exempt(account_info.lamports(), account_info.data_len()) {
        Err(SubscriptionError::NotRentExempt.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_derivation(
    ...
    if key != *account.key {
        return Err(SubscriptionError::DerivedKeyInvalid.into());
    }
    Ok(bump)
}
...
pub fn spl_token_transfer(params: TokenTransferParams<'_, '_>) -> ProgramResult {
    ...
    result.map_err(|_| SubscriptionError::TokenTransferFailed.into())
}
